...
- [x] **Phase 6 Strategy Fitter shipped (2026-05-24):** `fitStrategy(input, engineOut)` pure function · per-goal corpus + SIP allocation (greedy by rank) · horizon-keyed 4-bucket split · funded/partial/unfunded status · weighted bucket targets · action list (reserve · sip · allocate · flag · rebalance) · `useFittedStrategy` chained hook · live fitter view in the Engine tab (KPI tiles, bucket-target bar, action list, per-goal fit chip on every ranked card)
- [x] **Phase 7 dashboard rewiring shipped (2026-05-24):** GoalTracker · PlanExecutive · AssetAllocation now consume engine output instead of hand-coded baselines. GoalTracker shows engine-ranked goals with per-goal corpus + SIP + status from fitter. PlanExecutive actions come from `fit.actions` (was if/else). AssetAllocation target is derived from `fit.bucketTargets` (was hardcoded riskAppetite table). Each surface tagged "engine-driven" so the provenance is visible.
- [x] **Phase 8 Engine Explain dashboard shipped (2026-05-24):** new dashboard launcher under a new "Engine" category in the Dashboards menu. Four sections: (1) active configuration showing persona, confidence, money script, risk profile, weight derivation + criterion-weight bar chart; (2) pre-emption events listing each system goal that fired with its trigger; (3) per-goal derivation tables showing the full composite math (score × weight = contribution) for the top 8 ranked goals; (4) sensitivity vs equal-weight (25/25/25/25) baseline showing how the persona shifts each goal's rank — ↑boosted / ↓demoted indicators.
- [x] **Phase 9 snapshot + report integration shipped (2026-05-24):** snapshot persistence (`lib/orchestration/snapshot.ts`) writes EngineOutput + StrategyFit to localStorage on every hash change via `useFittedStrategy`. Format-agnostic adapter (`formatForReport.ts`) turns the snapshot into a structured report object. All 5 exporters now include the engine block: PDF (new §9.5 with weights · totals · top-8 ranked · top-5 actions), DOCX (full ranked-goals table + actions), PPTX (engine slide with persona strip + top-8 table + actions), Markdown (full tables under "## Orchestration Engine"), CSV (ranked-goals + actions rows). Exports auto-load latest snapshot if not explicitly provided.
